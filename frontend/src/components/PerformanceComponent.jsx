...
export default function PerformanceComponent({ allquestions, allAnswers, totalMark, maxMark, totalTime }) {
    ...
    return (<>
        ...
        <div className="h-auto w-full p-2 space-y-2 md:p-3">
            {allquestions && allquestions.map((question, q_index) => (
                <div key={q_index} className="w-full h-auto">
                    <div className="flex w-full h-auto overflow-hidden bg-white rounded-md">
                        <div className="pb-2 px-1.5 md:px-2 flex-1 h-auto overflow-hidden">
                            ...
                            <div className="w-full h-auto pl-4 mt-3">
                                <div className="h-auto w-full flex flex-wrap justify-start items-center gap-2 overflow-x-auto hidden-scrollbar text-xs *:flex *:items-center">
                                    {question.tags.map((tag, index) => (
                                        <p key={index} className="h-5 py-0.5 px-1.5 flex-none rounded-full bg-gray-100">{tag}</p>
                                    ))}
                                </div>
                            </div>
                        </div>
